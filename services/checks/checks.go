...
func New(agentsRegistry agentsRegistry, alertsRegistry alertRegistry, db *reform.DB) *Service {
	...
	s := &Service{
		...
		mExecutedScripts: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Name:      "check_script_executed_total",
...
func (s *Service) executeMySQLChecks(ctx context.Context) ([]string, error) {
	...
	for _, c := range checks {
		...
		for _, target := range targets {
			...
			s.mGeneratedAlerts.WithLabelValues(string(models.MySQLServiceType), string(c.Type)).Add(float64(len(alerts)))
...
func (s *Service) executePostgreSQLChecks(ctx context.Context) ([]string, error) {
	...
	for _, c := range checks {
		...
		for _, target := range targets {
			...
			s.mGeneratedAlerts.WithLabelValues(string(models.PostgreSQLServiceType), string(c.Type)).Add(float64(len(alerts)))
			res = append(res, alerts...)
		}
	}
...
func (s *Service) executeMongoDBChecks(ctx context.Context) ([]string, error) {
	...
	for _, c := range checks {
		...
		for _, target := range targets {
			...
			s.mGeneratedAlerts.WithLabelValues(string(models.MongoDBServiceType), string(c.Type)).Add(float64(len(alerts)))
			res = append(res, alerts...)
		}
	}
